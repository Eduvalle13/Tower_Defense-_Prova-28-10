...
public class Munição : MonoBehaviour
{
    ...
    [SerializeField] private float Velocidadetiro = 5f;//Defina a velocidade com que a munição se move em direção ao alvo
    ...
    private void FixedUpdate()//O método FixedUpdate() é usado em Unity para atualizar a física do objeto, sendo chamado em intervalos de tempo fixos.
    {   //Este método verifica se há um alvo (alvo). Se não houver, o método retorna e não executa mais código.
        if (!alvo) return;

        /*Se tiver um alvo, ele calcula a direção do projétil até o alvo. A direção é obtida 
         * subtraindo a posição do projétil (transform.position) da posição do alvo (alvo.position) e 
         * normalizando o vetor (para manter a direção sem modificar a magnitude). O resultado é multiplicado 
         * pela Velocidadetiro para definir a velocidade do projétil.*/
        Vector2 direcao = (alvo.position - transform.position).normalized;
        rigidbody2d.velocity = direcao * Velocidadetiro;//O rigidbody2d.velocity é atualizado com essa nova direção, o que movimenta o projétil fisicamente no espaço 2D em direção ao alvo.
    }

    private void OnCollisionEnter2D(Collision2D collision)//Este método é chamado quando o projétil colide com outro objeto. A função dele é verificar se o objeto com o qual o projétil colidiu é um inimigo e, caso seja, aplicar dano.
    {
    ...
    //Este método detecta colisões do projétil e, se ele colidir com um inimigo, aplica dano ao inimigo e destrói o projétil.
}