...
public class Monster : MonoBehaviour, IDamageble
{
    ...
    public void TakeDamage(int dano) //A função desse método é subtrair o valor dedanoda vida
    {
        ...
        if (vidaAtual <= 0) //O método verifica se a vida atual do inimigo(vidaAtual) é menor ou igual a 0.Se for, isso significa que o inimigo não tem mais vida suficiente para continuar no jogo.
        {
            Morrer();  // Chama o método Morrer quando a vida chegar a zero
        }
    }

    private void Morrer()//Ometodo Morrer. Lida com a remoção do inimigo do jogo, como a destruição do objeto,
    {
        SpawnManager.instance.InimigoDestruido();
        Debug.Log("Inimigo morreu!");  // Mensagem de depuração para confirmar a morte
